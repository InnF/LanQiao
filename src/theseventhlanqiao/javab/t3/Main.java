package theseventhlanqiao.javab.t3;
...
//        PrintWriter out = new PrintWriter(new BufferedOutputStream(System.out));
        ...
        for (int i = 1; i <= 9; i++) {
            for (int j = 1; j <= 9; j++) {
                ...
                for (int k = 1; k <= 9; k++) {
                    if (k == j || k == i)
                        continue;
                    for (int l = 0; l < hundreds.length; l++) {
                        if (isContainTheSameNumber(hundreds[l], i, j, k)) {
                            continue;
                        }
                        for (int m = 0; m < hundreds.length; m++) {
        ...
        out.println("used : " + used + " count : " + count);
        out.flush();
        out.close();
    }

    public static boolean isFormulaEstablish(int a, int b, int c, int def, int ghi) {
        return (a + (b / c) + (def / ghi)) == 10;
    }

    public static boolean isContainTheSameNumber(int num, Integer... n) {
        int b = num / 100;
        int s = (num / 10) % 10;
        int g = num % 10;
        for (Integer integer : n) {
            ...
            if (g == integer)
                return true;
            if (integer > 100) {
                if (String.valueOf(integer).contains(String.valueOf(b))
                        || String.valueOf(integer).contains(String.valueOf(s))
                        || String.valueOf(integer).contains(String.valueOf(g))) {
                    return true;
                }
            }
        }
